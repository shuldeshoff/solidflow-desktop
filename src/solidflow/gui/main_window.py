...
class MainWindow(QMainWindow):
    ...
    def _on_open(self):
        ...
        if file_name:
            try:
                self.viewport.load_mesh(file_name)
    ...
    def _on_save(self):
        ...
        if self.current_file:
            self.statusBar().showMessage("Сохранение (будет реализовано на Этапе 5)", 3000)

    def _on_save_as(self):
        """Обработчик сохранения файла как"""
        self.statusBar().showMessage(
            "Сохранение как (будет реализовано на Этапе 5)", 3000
        )
    ...
    def _update_info(self):
        ...
        if self.viewport.current_mesh is not None:
            ...
            info = f"""
<b>Файл:</b> {self.current_file.split('/')[-1]}<br>
<br>
<b>Геометрия:</b><br>
Треугольников: {mesh.n_cells}<br>
Вершин: {mesh.n_points}<br>
<br>
<b>Размеры:</b><br>
X: {bounds[1] - bounds[0]:.2f} мм<br>
Y: {bounds[3] - bounds[2]:.2f} мм<br>
Z: {bounds[5] - bounds[4]:.2f} мм<br>
"""
